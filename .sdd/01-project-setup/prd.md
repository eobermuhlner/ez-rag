...
- **Subcommand stubs**: `IngestCommand`, `QueryCommand`, `SearchCommand`, `StatusCommand`, `McpServerCommand`, `ShellCommand` — each a Spring-managed picocli `@Command` bean. In this PRD they are stubs; other PRDs implement them. (`SearchCommand` is specified in PRD 05 but stubbed here alongside the others.)
...
- Provider-specific Spring AI configuration — covered in PRD 02.
